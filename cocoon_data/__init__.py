...
def find_duplicate_indices(df):
    ...
    grouped = duplicates.groupby(list(duplicates.columns)).apply(lambda x: list(x.index))
...
test1 = get_source_nodes_ids({0: [1, 2], 1: [2], 2: [3]}, 2)
test2 = get_source_nodes_ids({0: [1, 3], 1: [2, 3], 2: [3], 3: [4]}, 3)
...
def display_workflow(nodes, edges):
    ...
    for tail, heads in edges.items():
        for head in heads:
            dot.edge(nodes[tail], nodes[head], **edge_style)
...
class DocumentedData:
    def __init__(self, file_path=None, df=None, table_name = None, log_file_path='data_log.txt', display_html=True):
...
And more to come! 🚀""")
    ...
    def check_document_exists(self):
        ...
        if self.table_name is not None:
            ...
            file_name = f"{data_name}_cocoon_data.json"
...
```"""
    ...
    def check_index_columns(self, overwrite=False, once=False):
        ...
        if len(index_column_indices) > 0:
            display_index_columns_html(self.df, index_column_indices)
...
def entity_relation_match(input_df, I, refernece_df, attributes=None, label = "label", match="matches"):
    ...
    while not emdeb_searcher.is_index_empty():
        ...
        for row in related_rows.iterrows():
            related_row_desc = ""
            for attribute in attributes:
                related_row_desc += (attribute + ": " + row[1][attribute] + "\n")
            related_rows_desc.append(related_row_desc)

        related_rows_desc_str = ""
        for i, related_row_desc in enumerate(related_rows_desc):
            related_rows_desc_str += (str(i+1) + ". " + related_row_desc + "\n")

        parsed_json_string = parse_match_result(json_var, reference_entities)

        json_var2 = find_similar_relation_match(input_desc = input_desc,
                                        refernece_desc = parsed_json_string, 
                                        related_rows_desc_str = related_rows_desc_str)
        
        indicies = json_var2["indices"]

        ids = []
        for index in indicies:
            ids.append(list(related_rows.index)[index-1])
...
def generate_page(df, i=0, js=[], exclude_columns=['label', 'index_ids', 'embedding', 'matches'], match_col='matches'):
    input_data_html = df_row_to_column_value(df, idx=i, exclude_columns=exclude_columns).to_html(index=False)
    similar_data_html = ""
    for j in clusters[i]:
