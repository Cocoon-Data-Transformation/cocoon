...
-- Generated at 2024-08-01 16:33:37.679702+00:00
WITH 
...
"payer_transitions_renamed_casted" AS (
    ...
    SELECT
        ...
        "owner_name",
        CASE
            WHEN regexp_full_match("coverage_end_date", '\d{4}-\d{2}-\d{2}T13:42:04Z') THEN CAST("coverage_end_date" AS TIMESTAMP)
            WHEN regexp_full_match("coverage_end_date", '\d{4}-\d{2}-\d{2}T03:41:07Z') THEN CAST("coverage_end_date" AS TIMESTAMP)
            WHEN regexp_full_match("coverage_end_date", '\d{4}-\d{2}-\d{2}T19:43:25Z') THEN CAST("coverage_end_date" AS TIMESTAMP)
        ...
        CAST("coverage_start_date" AS TIMESTAMP) 
